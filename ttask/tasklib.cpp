...
void TaskDelay::init(Robot* robo, aistate* ai) {
...
int TaskDelay::poll(Robot* robo, aistate* ai) {
...
int TaskTurnTo::poll(Robot* robo, aistate* ai) {
...
int TaskGo::poll(Robot* robo, aistate* ai) {
...
void TaskGoDur::init(Robot* robo, aistate* ai) {
    end_time += get_now_time();
    robo->set_vel(base_vel, base_vel);
}

int TaskGoDur::poll(Robot* robo, aistate* ai) {
...
int TaskMoveTowards::poll_inactive(Robot* robo, aistate* ai) {
...
int TaskMoveTowards::poll(Robot* robo, aistate* ai) {
...
void do_poll_tasks(Robot* robo, aistate* state) {
    ...
    for (size_t i = 0; i < state->task_stack.size() - 1 && !interrupted; i++) {
        ...
        int ipoll_res = at->task->poll_inactive(robo, state);
