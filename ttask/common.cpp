...
void RoboTask::init(Robot* robo, aistate* state) {
    //return TSTATUS_CONTINUE;
}

int RoboTask::poll_inactive(Robot* robo, aistate* state) {
    return TSTATUS_CONTINUE;
}

int RoboTask::poll(Robot* robo, aistate* state) {
    return TSTATUS_DONE;
}
...
aistate::aistate() : nav(Vec2i(GOAL_X, GOAL_Y)) {
    // nothing yet, I guess
}
